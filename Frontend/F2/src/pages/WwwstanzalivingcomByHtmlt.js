import "bootstrap/dist/css/bootstrap.min.css";
import { Button } from "react-bootstrap";
import "./WwwstanzalivingcomByHtmlt.css";

const WwwstanzalivingcomByHtmlt = () => {
  return (
    <div className="wwwstanzalivingcom-by-htmlt">
      <div className="div10">
        <div className="div11">
          <div className="divmask-group">
            <div className="div12">
              <div className="div13">
                <img
                  className="mg-2947-aa-1jpg-icon"
                  alt=""
                  src="../-mg-2947-aa-1jpg@2x.png"
                />
              </div>
            </div>
          </div>
          <div className="div14">
            <img
              className="play-newpng-icon"
              alt=""
              src="../playnewpng@2x.png"
            />
            <div className="video">Video</div>
          </div>
        </div>
        <div className="div15">
          <div className="div16">
            <div className="div17">
              <div className="div18">
                <div className="kyoto-house">Kyoto House</div>
              </div>
              <div className="south-campus">South Campus</div>
            </div>
            <img className="svgheart-icon" alt="" src="../svghearticon.svg" />
          </div>
          <div className="div19">
            <div className="div20">
              <img
                className="female-listingpng-icon"
                alt=""
                src="../femalelistingpng@2x.png"
              />
              <div className="female">Female</div>
            </div>
            <div className="div21" />
            <div className="div22">
              <img
                className="female-listingpng-icon"
                alt=""
                src="../occupancypng@2x.png"
              />
              <div className="double-triple">Double, Triple</div>
            </div>
            <div className="div23">
              <img
                className="directionspng-icon"
                alt=""
                src="../directionspng@2x.png"
              />
              <div className="view-directions">View Directions</div>
            </div>
          </div>
          <div className="div24">
            <div className="amenities">Amenities</div>
            <div className="div25">
              <div className="div26">
                <img className="attached-balconypng-icon" alt="" />
                <div className="attached-balcony">Attached Balcony</div>
              </div>
              <Button className="div27" variant="primary">
                Air Conditioning
              </Button>
              <div className="div28">
                <img
                  className="attached-washroompng-icon"
                  alt=""
                  src="../attached-washroompng@2x.png"
                />
                <div className="attached-washroom">Attached Washroom</div>
              </div>
              <div className="div29">
                <img
                  className="small-cupboardpng-icon"
                  alt=""
                  src="../small-cupboardpng@2x.png"
                />
                <div className="spacious-cupboard">Spacious Cupboard</div>
              </div>
              <div className="div30">
                <div className="div31">+1</div>
              </div>
            </div>
          </div>
          <div className="div32">
            <div className="div33">
              <div className="amenities">Starts from</div>
              <div className="mo">
                <span>₹13,799</span>
                <span className="mo1">/mo*</span>
              </div>
            </div>
            <div className="div34">
              <div className="button1">
                <div className="schedule-a-visit">Schedule a visit</div>
              </div>
              <div className="button2">
                <div className="reserve-now">Reserve Now</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="div-next">
        <a className="div35">
          <div className="div36">
            <div className="div37">
              <div className="div38">
                <div className="div39">
                  <div className="locality">Locality</div>
                  <img className="svg-icon" alt="" src="../svg.svg" />
                </div>
                <div className="div40">
                  <div className="locality">Budget</div>
                  <img className="svg-icon" alt="" src="../svg1.svg" />
                </div>
                <div className="div41">
                  <div className="locality">Occupancy</div>
                  <img className="svg-icon" alt="" src="../svg2.svg" />
                </div>
                <div className="div42">
                  <div className="locality">Gender</div>
                  <img className="svg-icon" alt="" src="../svg3.svg" />
                </div>
                <div className="div43">
                  <div className="locality">Amenities</div>
                  <img className="svg-icon" alt="" src="../svg4.svg" />
                </div>
              </div>
              <div className="div44" />
              <div className="div45">
                <div className="locality">My Wishlist</div>
              </div>
              <div className="div46">
                <img
                  className="more-filterspng-icon"
                  alt=""
                  src="../morefilterspng@2x.png"
                />
                <div className="more-filters">More Filters</div>
              </div>
              <div className="div47">
                <div className="sort-by">Sort By:</div>
                <div className="popularity">Popularity</div>
                <img className="svg-icon5" alt="" src="../svg5.svg" />
              </div>
            </div>
            <div className="pgs-found-near">53 PG’s found near JSS</div>
            <div className="div48">
              <div className="div49">
                <div className="div50">
                  <div className="div11">
                    <div className="divmask-group1">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../3jpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng1@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div56">
                      <div className="div17">
                        <div className="div58">
                          <div className="kyoto-house">Boston House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon1.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div62">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng1@2x.png"
                        />
                        <div className="double">Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng1@2x.png"
                        />
                        <div className="view-directions1">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div66">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div67">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng1@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div68">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng1@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div70">
                      <div className="div71">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹20,699</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div73">
                  <div className="div74">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div76">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../jkeiq0ltnldwhapbnzgcjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div78">
                    <div className="div56">
                      <div className="div17">
                        <div className="div81">
                          <div className="span5" />
                          <div className="nagoya-house">Nagoya House</div>
                          <div className="almost-full">Almost Full</div>
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon2.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div85">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng1@2x.png"
                        />
                        <div className="single-double">Single, Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng1@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div88">
                        <div className="div89">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div90">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng1@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div91">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng1@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div92">
                      <div className="div93">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹16,799</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div95">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../bsdpt6qv4rd6ldhkhcwrjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div103">
                          <div className="kyoto-house">Montreal House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon3.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div22">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div66">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng2@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div67">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng3@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div68">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng3@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng1@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div115">
                      <div className="div116">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹16,199</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div118">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../ke7zd9ob4nkqi2xoxdgsjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng4@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Polaris House</div>
                          <div className="span6" />
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon4.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng2@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div22">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng4@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng4@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng3@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng4@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng4@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div92">
                      <div className="div93">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹16,799</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div141">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../-mg-2947-aa-1jpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Tokyo House</div>
                          <div className="span7" />
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon5.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div22">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng1@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng4@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div30">
                          <div className="div31">+1</div>
                        </div>
                      </div>
                    </div>
                    <div className="div163">
                      <div className="div164">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹15,299</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div166">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../ncokwqwvsbi1wxscfe1tjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Cardiff House</div>
                          <div className="span8" />
                        </div>
                        <div className="south-campus">Laxmi Nagar</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon6.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div62">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng1@2x.png"
                        />
                        <div className="double">Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div66">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng2@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div67">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng3@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div68">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng3@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng1@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div186">
                      <div className="div187">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹11,599</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div189">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../-mg-2947-aa-1jpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div18">
                          <div className="kyoto-house">Kyoto House</div>
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon7.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div22">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng1@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng4@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div30">
                          <div className="div31">+1</div>
                        </div>
                      </div>
                    </div>
                    <div className="div32">
                      <div className="div33">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹13,799</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div214">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../ncokwqwvsbi1wxscfe1tjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Cardiff House</div>
                          <div className="span8" />
                        </div>
                        <div className="south-campus">Laxmi Nagar</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon8.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div62">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng1@2x.png"
                        />
                        <div className="double">Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div66">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng2@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div67">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng3@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div68">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng3@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng1@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div186">
                      <div className="div187">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹11,599</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div237">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../jaovmzbj7buontqjtw8fjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Poznan House</div>
                          <div className="span10" />
                        </div>
                        <div className="south-campus">Laxmi Nagar</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon9.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div249">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng@2x.png"
                        />
                        <div className="double">Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng1@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng4@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div255">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng9@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div256">
                          <img
                            className="shared-washroompng-icon"
                            alt=""
                            src="../shared-washroompng@2x.png"
                          />
                          <div className="shared-washroom">Shared Washroom</div>
                        </div>
                        <div className="div257">
                          <div className="div31">+1</div>
                        </div>
                      </div>
                    </div>
                    <div className="div259">
                      <div className="div260">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹10,299</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div262">
                  <div className="div263">
                    <div className="divmask-group10">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../v78jccm6iwxpwwvsvgtrjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div266">
                      <div className="div267">
                        <img
                          className="play-newpng-icon"
                          alt=""
                          src="../playnewpng4@2x.png"
                        />
                        <div className="video">Video</div>
                      </div>
                      <div className="div268">
                        <img
                          className="play-newpng-icon"
                          alt=""
                          src="../3dtournewpng@2x.png"
                        />
                        <div className="d-tour">3D Tour</div>
                      </div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Perugia House</div>
                          <div className="span11" />
                        </div>
                        <div className="south-campus">Laxmi Nagar</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon10.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng2@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div276">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng4@2x.png"
                        />
                        <div className="single-double-triple">
                          Single, Double, Triple, Quadruple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng4@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div281">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng9@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div282">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng10@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng4@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div284">
                      <div className="div285">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹8,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div287">
                  <div className="div11">
                    <div className="divmask-group1">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../zngdrn1a0ezh6ud4uf9ajpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng4@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div293">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Quito House</div>
                          <div className="span12" />
                        </div>
                        <div className="south-campus">Laxmi Nagar</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon11.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng4@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div299">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng11@2x.png"
                        />
                        <div className="single-double-triple1">
                          Single, Double, Triple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng4@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng3@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng4@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng4@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div30">
                          <div className="div308">+2</div>
                        </div>
                      </div>
                    </div>
                    <div className="div309">
                      <div className="div310">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹7,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div312">
                  <div className="div11">
                    <div className="divmask-group1">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../zgb83wsz58f8ntsoqunyjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng12@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Kiel House</div>
                          <div className="span13" />
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon12.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng7@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div324">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng12@2x.png"
                        />
                        <div className="double-triple-quadruple">
                          Double, Triple, Quadruple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng12@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng6@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng10@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng11@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng12@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div332">
                      <div className="div333">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹13,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div335">
                  <div className="div74">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div76">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../s4dyhrmykshvfh8aay0gjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng12@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div78">
                    <div className="div16">
                      <div className="div17">
                        <div className="div343">
                          <div className="kyoto-house">Kobe House</div>
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon13.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng7@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div347">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng12@2x.png"
                        />
                        <div className="single-double">Single, Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng12@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div88">
                        <div className="div89">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng11@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div90">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng12@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div91">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng13@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div354">
                      <div className="div355">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹15,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div357">
                  <div className="div74">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div76">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../alh5kw6isikvzu1wuqadjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng12@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div78">
                    <div className="div16">
                      <div className="div17">
                        <div className="div365">
                          <div className="kyoto-house">Sheffield House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon14.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng5@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div369">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng14@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng12@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div372">
                        <div className="div90">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng13@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div91">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng14@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div375">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng5@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div376">
                      <div className="div377">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹18,299</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div379">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../fcxgdg1dc9ltzgo2miksjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng12@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Osaka House</div>
                          <div className="span14" />
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon15.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng7@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div249">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng12@2x.png"
                        />
                        <div className="double">Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng12@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div66">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng11@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div67">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng12@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div68">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng13@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng6@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div399">
                      <div className="div400">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹15,899</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div402">
                  <div className="div11">
                    <div className="divmask-group1">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../dihpoa8fdyx6eptagh3rjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng12@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div410">
                          <div className="kyoto-house">Penzance House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon16.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng7@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div22">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng12@2x.png"
                        />
                        <div className="double-triple">Double, Triple</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng12@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng6@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng10@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng11@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng12@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div30">
                          <div className="div31">+1</div>
                        </div>
                      </div>
                    </div>
                    <div className="div424">
                      <div className="div425">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹14,799</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div427">
                  <div className="div74">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div76">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../zrfwrzmvbl8uoozvev5zjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng17@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div78">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Hiroshima House</div>
                          <div className="span15" />
                        </div>
                        <div className="south-campus">South Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon17.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng6@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div85">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng17@2x.png"
                        />
                        <div className="single-double">Single, Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng17@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div88">
                        <div className="div89">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng14@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div90">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng16@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div91">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng17@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div354">
                      <div className="div355">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹15,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div449">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../-mg-2864-aa-1jpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng17@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div457">
                          <div className="kyoto-house">Toronto House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon18.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng11@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div461">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng18@2x.png"
                        />
                        <div className="single-double-triple1">
                          Single, Double, Triple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng17@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng8@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng15@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng17@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng18@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div30">
                          <div className="div31">+1</div>
                        </div>
                      </div>
                    </div>
                    <div className="div471">
                      <div className="div472">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹14,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div474">
                  <div className="div74">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div76">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../fxdtwl2hiqjc9gcjzjncjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng17@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div78">
                    <div className="div16">
                      <div className="div17">
                        <div className="div81">
                          <div className="nagoya-house">Romford House</div>
                          <div className="span16" />
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon19.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div60">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../malelistingpng6@2x.png"
                        />
                        <div className="male">Male</div>
                      </div>
                      <div className="div61" />
                      <div className="div299">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng17@2x.png"
                        />
                        <div className="single-double-triple1">
                          Single, Double, Triple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng17@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div372">
                        <div className="div90">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng18@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div91">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng19@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div375">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng7@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div493">
                      <div className="div494">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹17,299</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div496">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../iplhchibapoabl3htyjdjpg@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng17@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div17">
                        <div className="div365">
                          <div className="kyoto-house">Stockton House</div>
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon20.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng11@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div347">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng18@2x.png"
                        />
                        <div className="single-double">Single, Double</div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng17@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng8@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div281">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng19@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div282">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng20@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                        <div className="div69">
                          <img
                            className="windowpng-icon"
                            alt=""
                            src="../windowpng8@2x.png"
                          />
                          <div className="window">Window</div>
                        </div>
                      </div>
                    </div>
                    <div className="div516">
                      <div className="div33">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹19,699</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div519">
                  <div className="div11">
                    <div className="divmask-group">
                      <div className="div12">
                        <div className="div13">
                          <img
                            className="mg-2947-aa-1jpg-icon"
                            alt=""
                            src="../zgb83wsz58f8ntsoqunyjpg1@2x.png"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div14">
                      <img
                        className="play-newpng-icon"
                        alt=""
                        src="../playnewpng@2x.png"
                      />
                      <div className="video">Video</div>
                    </div>
                  </div>
                  <div className="div15">
                    <div className="div16">
                      <div className="div526">
                        <div className="div81">
                          <div className="nagoya-house">Kiel House</div>
                          <div className="span13" />
                        </div>
                        <div className="south-campus">North Campus</div>
                      </div>
                      <img
                        className="svgheart-icon"
                        alt=""
                        src="../svghearticon21.svg"
                      />
                    </div>
                    <div className="div19">
                      <div className="div20">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../femalelistingpng@2x.png"
                        />
                        <div className="female">Female</div>
                      </div>
                      <div className="div21" />
                      <div className="div324">
                        <img
                          className="female-listingpng-icon"
                          alt=""
                          src="../occupancypng@2x.png"
                        />
                        <div className="double-triple-quadruple">
                          Double, Triple, Quadruple
                        </div>
                      </div>
                      <div className="div23">
                        <img
                          className="directionspng-icon"
                          alt=""
                          src="../directionspng@2x.png"
                        />
                        <div className="view-directions">View Directions</div>
                      </div>
                    </div>
                    <div className="div24">
                      <div className="amenities">Amenities</div>
                      <div className="div25">
                        <div className="div134">
                          <img
                            className="attached-balconypng-icon"
                            alt=""
                            src="../attached-balconypng1@2x.png"
                          />
                          <div className="attached-balcony">
                            Attached Balcony
                          </div>
                        </div>
                        <div className="div135">
                          <img
                            className="acpng-icon"
                            alt=""
                            src="../acpng4@2x.png"
                          />
                          <div className="air-conditioning">
                            Air Conditioning
                          </div>
                        </div>
                        <div className="div28">
                          <img
                            className="attached-washroompng-icon"
                            alt=""
                            src="../attached-washroompng@2x.png"
                          />
                          <div className="attached-washroom">
                            Attached Washroom
                          </div>
                        </div>
                        <div className="div29">
                          <img
                            className="small-cupboardpng-icon"
                            alt=""
                            src="../small-cupboardpng@2x.png"
                          />
                          <div className="spacious-cupboard">
                            Spacious Cupboard
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div332">
                      <div className="div333">
                        <div className="amenities">Starts from</div>
                        <div className="mo">
                          <span>₹13,499</span>
                          <span className="mo1">/mo*</span>
                        </div>
                      </div>
                      <div className="div34">
                        <div className="button1">
                          <div className="schedule-a-visit">
                            Schedule a visit
                          </div>
                        </div>
                        <div className="button2">
                          <div className="reserve-now">Reserve Now</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="div-wrapper">
                <div className="div542">
                  <div className="div543">
                    <div className="div544">
                      <div className="div545">
                        <div className="map">
                          <div className="map">
                            <img className="div-icon" alt="" src="../div.svg" />
                            <div className="div547">
                              <div className="map">
                                <div className="div549">
                                  <img
                                    className="transparentpng-icon"
                                    alt=""
                                    src="../transparentpng@2x.png"
                                  />
                                </div>
                                <div className="div550">
                                  <img
                                    className="transparentpng-icon"
                                    alt=""
                                    src="../transparentpng@2x.png"
                                  />
                                </div>
                                <div className="div551">
                                  <img
                                    className="transparentpng-icon"
                                    alt=""
                                    src="../transparentpng@2x.png"
                                  />
                                </div>
                                <div className="div552">
                                  <img
                                    className="transparentpng-icon"
                                    alt=""
                                    src="../transparentpng@2x.png"
                                  />
                                </div>
                              </div>
                              <div className="div553">
                                <div className="div554">
                                  <div className="div555">
                                    <div className="div556">
                                      <img
                                        className="jpg-icon1"
                                        alt=""
                                        src="../3jpg1@2x.png"
                                      />
                                      <div className="div557">
                                        <div className="div558">
                                          <div className="boston-house1">
                                            Boston House
                                          </div>
                                        </div>
                                        <div className="starts-from22">
                                          Starts from :
                                        </div>
                                        <div className="month">
                                          ₹20,699 / month*
                                        </div>
                                        <div className="div559">
                                          <div className="north-campus10">
                                            North Campus
                                          </div>
                                          <div className="divafter" />
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                                <div className="divafter1" />
                              </div>
                            </div>
                          </div>
                          <img className="div-icon1" alt="" src="../div1.svg" />
                          <div className="div560">
                            <div className="div561">
                              <div className="div562">
                                <div className="div563" />
                              </div>
                              <div className="report-a-map">
                                Report a map error
                              </div>
                            </div>
                            <div className="div564">
                              <div className="div562">
                                <div className="div566" />
                              </div>
                              <div className="keyboard-shortcuts">
                                Keyboard shortcuts
                              </div>
                            </div>
                            <div className="div567">
                              <div className="div562">
                                <div className="div569" />
                              </div>
                              <div className="report-a-map">Map data ©2023</div>
                            </div>
                            <div className="div570">
                              <div className="div562">
                                <div className="div572" />
                              </div>
                              <div className="report-a-map">Terms of Use</div>
                            </div>
                          </div>
                          <img className="div-icon2" alt="" src="../div2.svg" />
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="div573">
                <img
                  className="whatsapppng-icon"
                  alt=""
                  src="../whatsapppng@2x.png"
                />
              </div>
            </div>
            <div className="filters">Filters</div>
          </div>
          <div className="div574">
            <div className="div575">
              <div className="popular-searches">Popular Searches</div>
              <div className="ulmagic-quad-scroll">
                <div className="a4">
                  <div className="pg-in-greater">PG in Greater Noida</div>
                </div>
                <div className="a5">
                  <div className="pg-in-bengaluru">PG in Bengaluru</div>
                </div>
                <div className="a6">
                  <div className="pg-in-vadodara">PG in Vadodara</div>
                </div>
                <div className="a7">
                  <div className="pg-in-dehradun">PG in Dehradun</div>
                </div>
                <div className="a8">
                  <div className="pg-in-indore">PG in Indore</div>
                </div>
                <div className="a9">
                  <div className="pg-in-pune">PG in Pune</div>
                </div>
                <div className="a10">
                  <div className="pg-in-hyderabad">PG in Hyderabad</div>
                </div>
                <div className="a11">
                  <div className="pg-in-coimbatore">PG in Coimbatore</div>
                </div>
                <div className="a12">
                  <div className="pg-in-ahmedabad">PG in Ahmedabad</div>
                </div>
                <div className="a13">
                  <div className="pg-in-visakhapatnam">PG in Visakhapatnam</div>
                </div>
                <div className="a14">
                  <div className="pg-in-vidyanagar">PG in Vidyanagar</div>
                </div>
                <div className="a15">
                  <div className="pg-in-manipal">PG in Manipal</div>
                </div>
                <div className="a16">
                  <div className="pg-in-kochi">PG in Kochi</div>
                </div>
                <div className="a17">
                  <div className="pg-in-gandhinagar">PG in Gandhinagar</div>
                </div>
                <div className="a18">
                  <div className="pg-in-jaipur">PG in Jaipur</div>
                </div>
                <div className="a19">
                  <div className="pg-in-nagpur">PG in Nagpur</div>
                </div>
                <div className="a20">
                  <div className="pg-in-vijayawada">PG in Vijayawada</div>
                </div>
                <div className="a21">
                  <div className="pg-in-chennai">PG in Chennai</div>
                </div>
                <div className="a22">
                  <div className="pg-in-belgaum">PG in Belgaum</div>
                </div>
                <div className="a23">
                  <div className="pg-in-kota">PG in Kota</div>
                </div>
                <div className="a24">
                  <div className="pg-in-mumbai">PG in Mumbai</div>
                </div>
                <div className="a25">
                  <div className="pg-in-gurgaon">PG in Gurgaon</div>
                </div>
                <div className="a26">
                  <div className="pg-in-south">PG in South Campus</div>
                </div>
                <div className="a27">
                  <div className="pg-in-north">PG in North Campus</div>
                </div>
                <div className="a28">
                  <div className="pg-in-laxmi">PG in Laxmi Nagar</div>
                </div>
              </div>
            </div>
          </div>
          <img className="div-child" alt="" />
        </a>
        <div className="div576">
          <div className="div577">
            <div className="ul1">
              <div className="about-us1">About Us</div>
              <div className="team">Team</div>
              <div className="investor-relations">Investor Relations</div>
              <div className="media">Media</div>
              <div className="blogs">Blogs</div>
              <div className="faqs">FAQs</div>
              <div className="refer-and-earn">Refer and Earn</div>
              <div className="house-rules">House Rules</div>
              <div className="tc">{`T&C`}</div>
              <div className="privacy-policy1">Privacy Policy</div>
              <div className="careers">Careers</div>
              <div className="contact-us2">Contact Us</div>
              <div className="covid-19">COVID-19</div>
              <div className="refunds">Refunds</div>
              <div className="partner-with-us">Partner With Us</div>
              <div className="cookie-policy">Cookie Policy</div>
            </div>
            <div className="div578">
              <img className="fbpng-icon" alt="" src="../fbpng@2x.png" />
              <img className="linkpng-icon" alt="" src="../linkpng@2x.png" />
              <img
                className="vectorpng-icon"
                alt=""
                src="../vectorpng@2x.png"
              />
            </div>
            <img
              className="stanzalogopng-icon"
              alt=""
              src="../stanzalogopng@2x.png"
            />
          </div>
          <div className="div579">
            <div className="copyright-2022">
              Copyright © 2022 | All Rights Reserved by Dtwelve Spaces Pvt Ltd.
              | Sitemap
            </div>
            <div className="images-shown-are">
              Images shown are for representational purposes only. Amenities
              depicted may or may not form a part of that individual property.
            </div>
          </div>
        </div>
        <img className="div-next-child" alt="" src="../group-1322.svg" />
        <div className="locate3">Locate</div>
        <div className="about2">About</div>
        <div className="contact-us3">Contact us</div>
        <div className="div-next-inner">
          <div className="group-wrapper417">
            <div className="group-wrapper417">
              <div className="group-wrapper417">
                <div className="group-wrapper417">
                  <div className="group-wrapper417">
                    <div className="group-wrapper417">
                      <div className="group-wrapper417">
                        <div className="group-wrapper417">
                          <div className="group-wrapper417">
                            <div className="group-wrapper417">
                              <div className="group-wrapper417">
                                <div className="group-wrapper417">
                                  <div className="group-wrapper417">
                                    <div className="group-wrapper417">
                                      <div className="group-wrapper417">
                                        <div className="group-wrapper417">
                                          <div className="group-wrapper417">
                                            <div className="group-wrapper417">
                                              <div className="group-wrapper417">
                                                <div className="group-wrapper417">
                                                  <div className="group-wrapper417">
                                                    <div className="group-wrapper417">
                                                      <div className="group-wrapper417">
                                                        <div className="group-wrapper417">
                                                          <div className="group-wrapper417">
                                                            <div className="group-wrapper417">
                                                              <div className="group-wrapper417">
                                                                <div className="group-wrapper417">
                                                                  <div className="group-wrapper417">
                                                                    <div className="group-wrapper417">
                                                                      <div className="group-wrapper417">
                                                                        <div className="group-wrapper417">
                                                                          <div className="group-wrapper417">
                                                                            <div className="group-wrapper417">
                                                                              <div className="group-wrapper417">
                                                                                <div className="group-wrapper417">
                                                                                  <div className="group-wrapper417">
                                                                                    <div className="group-wrapper417">
                                                                                      <div className="group-wrapper417">
                                                                                        <div className="group-wrapper417">
                                                                                          <div className="group-wrapper417">
                                                                                            <div className="group-wrapper417">
                                                                                              <div className="group-wrapper417">
                                                                                                <div className="group-wrapper459">
                                                                                                  <div className="group-wrapper460">
                                                                                                    <div className="group-wrapper460">
                                                                                                      <div className="group-wrapper460">
                                                                                                        <div className="group-wrapper460">
                                                                                                          <div className="group-wrapper460">
                                                                                                            <div className="group-wrapper460">
                                                                                                              <div className="group-wrapper460">
                                                                                                                <div className="group-wrapper460">
                                                                                                                  <div className="group-wrapper460">
                                                                                                                    <div className="group-wrapper460">
                                                                                                                      <div className="group-wrapper460">
                                                                                                                        <div className="group-wrapper460">
                                                                                                                          <div className="group-wrapper460">
                                                                                                                            <div className="group-wrapper460">
                                                                                                                              <div className="group-wrapper460">
                                                                                                                                <div className="group-wrapper460">
                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                                                                        <div className="hostel-hopper4">
                                                                                                                                                                                                                          Hostel
                                                                                                                                                                                                                          Hopper
                                                                                                                                                                                                                        </div>
                                                                                                                                                                                                                      </div>
                                                                                                                                                                                                                    </div>
                                                                                                                                                                                                                  </div>
                                                                                                                                                                                                                </div>
                                                                                                                                                                                                              </div>
                                                                                                                                                                                                            </div>
                                                                                                                                                                                                          </div>
                                                                                                                                                                                                        </div>
                                                                                                                                                                                                      </div>
                                                                                                                                                                                                    </div>
                                                                                                                                                                                                  </div>
                                                                                                                                                                                                </div>
                                                                                                                                                                                              </div>
                                                                                                                                                                                            </div>
                                                                                                                                                                                          </div>
                                                                                                                                                                                        </div>
                                                                                                                                                                                      </div>
                                                                                                                                                                                    </div>
                                                                                                                                                                                  </div>
                                                                                                                                                                                </div>
                                                                                                                                                                              </div>
                                                                                                                                                                            </div>
                                                                                                                                                                          </div>
                                                                                                                                                                        </div>
                                                                                                                                                                      </div>
                                                                                                                                                                    </div>
                                                                                                                                                                  </div>
                                                                                                                                                                </div>
                                                                                                                                                              </div>
                                                                                                                                                            </div>
                                                                                                                                                          </div>
                                                                                                                                                        </div>
                                                                                                                                                      </div>
                                                                                                                                                    </div>
                                                                                                                                                  </div>
                                                                                                                                                </div>
                                                                                                                                              </div>
                                                                                                                                            </div>
                                                                                                                                          </div>
                                                                                                                                        </div>
                                                                                                                                      </div>
                                                                                                                                    </div>
                                                                                                                                  </div>
                                                                                                                                </div>
                                                                                                                              </div>
                                                                                                                            </div>
                                                                                                                          </div>
                                                                                                                        </div>
                                                                                                                      </div>
                                                                                                                    </div>
                                                                                                                  </div>
                                                                                                                </div>
                                                                                                              </div>
                                                                                                            </div>
                                                                                                          </div>
                                                                                                        </div>
                                                                                                      </div>
                                                                                                    </div>
                                                                                                  </div>
                                                                                                </div>
                                                                                                <img
                                                                                                  className="bunk-1-icon3"
                                                                                                  alt=""
                                                                                                  src="../bunk-1@2x.png"
                                                                                                />
                                                                                                <div className="group-wrapper518">
                                                                                                  <div className="group-wrapper518">
                                                                                                    <div className="group-wrapper518">
                                                                                                      <div className="group-wrapper518">
                                                                                                        <div className="group-wrapper518">
                                                                                                          <div className="group-wrapper518">
                                                                                                            <div className="group-wrapper518">
                                                                                                              <div className="group-wrapper518">
                                                                                                                <div className="group-wrapper518">
                                                                                                                  <div className="group-wrapper518">
                                                                                                                    <div className="group-wrapper518">
                                                                                                                      <div className="group-wrapper518">
                                                                                                                        <div className="group-wrapper518">
                                                                                                                          <div className="group-wrapper518">
                                                                                                                            <div className="group-wrapper518">
                                                                                                                              <div className="group-wrapper518">
                                                                                                                                <div className="group-wrapper518">
                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                            <div className="group-wrapper518">
                                                                                                                                              <div className="group-wrapper518">
                                                                                                                                                <div className="group-wrapper518">
                                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                                            <div className="group-wrapper518">
                                                                                                                                                              <div className="group-wrapper518">
                                                                                                                                                                <div className="group-wrapper518">
                                                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                                                            <div className="group-wrapper518">
                                                                                                                                                                              <div className="group-wrapper518">
                                                                                                                                                                                <div className="group-wrapper518">
                                                                                                                                                                                  <img
                                                                                                                                                                                    className="pngwing-1-icon3"
                                                                                                                                                                                    alt=""
                                                                                                                                                                                    src="../pngwing-1@2x.png"
                                                                                                                                                                                  />
                                                                                                                                                                                </div>
                                                                                                                                                                              </div>
                                                                                                                                                                            </div>
                                                                                                                                                                          </div>
                                                                                                                                                                        </div>
                                                                                                                                                                      </div>
                                                                                                                                                                    </div>
                                                                                                                                                                  </div>
                                                                                                                                                                </div>
                                                                                                                                                              </div>
                                                                                                                                                            </div>
                                                                                                                                                          </div>
                                                                                                                                                        </div>
                                                                                                                                                      </div>
                                                                                                                                                    </div>
                                                                                                                                                  </div>
                                                                                                                                                </div>
                                                                                                                                              </div>
                                                                                                                                            </div>
                                                                                                                                          </div>
                                                                                                                                        </div>
                                                                                                                                      </div>
                                                                                                                                    </div>
                                                                                                                                  </div>
                                                                                                                                </div>
                                                                                                                              </div>
                                                                                                                            </div>
                                                                                                                          </div>
                                                                                                                        </div>
                                                                                                                      </div>
                                                                                                                    </div>
                                                                                                                  </div>
                                                                                                                </div>
                                                                                                              </div>
                                                                                                            </div>
                                                                                                          </div>
                                                                                                        </div>
                                                                                                      </div>
                                                                                                    </div>
                                                                                                  </div>
                                                                                                </div>
                                                                                              </div>
                                                                                            </div>
                                                                                          </div>
                                                                                        </div>
                                                                                      </div>
                                                                                    </div>
                                                                                  </div>
                                                                                </div>
                                                                              </div>
                                                                            </div>
                                                                          </div>
                                                                        </div>
                                                                      </div>
                                                                    </div>
                                                                  </div>
                                                                </div>
                                                              </div>
                                                            </div>
                                                          </div>
                                                        </div>
                                                      </div>
                                                    </div>
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="group-parent6">
          <div className="rectangle-container">
            <div className="group-child1" />
            <div className="search-for-files1">
              Search for files, plugins, and creators
            </div>
          </div>
          <img className="group-child2" alt="" src="../group-152.svg" />
          <div className="search3">search</div>
        </div>
      </div>
      <div className="wwwbookingcom-by-htmltodes">
        <div className="divbodyconstraint-inner">
          <div className="ol">
            <div className="home">Home</div>
            <img className="svg-icon6" alt="" src="../svg6.svg" />
            <div className="div580">
              <div className="all-hostels">All hostels</div>
              <div className="span18">
                <div className="hostels">Hostels</div>
                <img className="svg-icon7" alt="" src="../svg7.svg" />
              </div>
            </div>
            <img className="svg-icon8" alt="" src="../svg8.svg" />
            <div className="div581">
              <div className="all-hostels">India</div>
              <div className="span19">
                <div className="hostels">Hostels</div>
                <img className="svg-icon7" alt="" src="../svg9.svg" />
              </div>
            </div>
            <img className="svg-icon10" alt="" src="../svg10.svg" />
            <div className="div582">
              <div className="all-hostels">Delhi NCR</div>
              <div className="span19">
                <div className="hostels">Hostels</div>
                <img className="svg-icon7" alt="" src="../svg11.svg" />
              </div>
            </div>
            <img className="svg-icon12" alt="" src="../svg12.svg" />
            <div className="div583">
              <div className="all-hostels">New Delhi</div>
              <div className="span19">
                <div className="hostels">Hostels</div>
                <img className="svg-icon7" alt="" src="../svg13.svg" />
              </div>
            </div>
            <img className="svg-icon14" alt="" src="../svg14.svg" />
            <div className="a-sleepy-fox">
              A Sleepy fox (Hostel), New Delhi (India) deals
            </div>
          </div>
          <div className="div584">
            <div className="div585">
              <div className="svg">
                <img className="frame-icon" alt="" src="../frame.svg" />
              </div>
              <b className="coronavirus-covid-19-support">
                Coronavirus (COVID-19) Support
              </b>
              <div className="svg1">
                <img className="frame-icon" alt="" src="../frame1.svg" />
              </div>
            </div>
          </div>
          <div className="divhoteltmpl">
            <div className="div586">
              <div className="div587">
                <div className="div588">
                  <div className="div589">
                    <div className="button45">
                      <div className="span22">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame2.svg"
                          />
                        </div>
                      </div>
                      <div className="we-price-match">We Price Match</div>
                    </div>
                  </div>
                  <div className="div590">
                    <div className="search4">Search</div>
                    <div className="div591">
                      <div className="all-hostels">
                        Destination/property name:
                      </div>
                      <div className="div592">
                        <div className="div593" />
                        <div className="div594">
                          <div className="span22">
                            <img
                              className="frame-icon"
                              alt=""
                              src="../frame3.svg"
                            />
                          </div>
                          <div className="input">
                            <div className="div595">
                              <div className="new-delhi1">New Delhi</div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="div596">
                      <div className="check-in-date">Check-in date</div>
                      <div className="button46">
                        <div className="svg4">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame4.svg"
                          />
                        </div>
                        <div className="div597">+</div>
                        <div className="check-in-date1">Check-in date</div>
                        <div className="svg5">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame5.svg"
                          />
                        </div>
                      </div>
                      <div className="check-out-date">Check-out date</div>
                      <div className="button47">
                        <div className="svg4">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame4.svg"
                          />
                        </div>
                        <div className="div597">+</div>
                        <div className="check-in-date1">Check-out date</div>
                        <div className="svg5">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame5.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="button48">
                      <div className="adults-0">
                        2 adults · 0 children · 1 room
                      </div>
                      <div className="svg5">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame5.svg"
                        />
                      </div>
                    </div>
                    <div className="div599">
                      <div className="label">
                        <div className="im-travelling-for">
                          I'm travelling for work
                        </div>
                        <div className="span23" />
                      </div>
                      <div className="svg9">
                        <img
                          className="frame-icon9"
                          alt=""
                          src="../frame9.svg"
                        />
                      </div>
                    </div>
                    <div className="button49">
                      <div className="pseudo" />
                      <b className="search5">Search</b>
                    </div>
                  </div>
                  <div className="ahotel-sidebar-static-map">
                    <img className="svg-icon15" alt="" src="../svg15.svg" />
                    <div className="button50">
                      <div className="pseudo1" />
                      <div className="show-on-map">Show on map</div>
                    </div>
                  </div>
                </div>
                <div className="div600">
                  <div className="div601">
                    <div className="ul2">
                      <div className="li">
                        <div className="we-price-match">{`Info & prices`}</div>
                      </div>
                      <div className="li1">
                        <div className="we-price-match">Amenities</div>
                      </div>
                      <div className="li2">
                        <div className="we-price-match">House rules</div>
                      </div>
                      <div className="guest-reviews-22">Guest reviews (22)</div>
                    </div>
                  </div>
                  <div className="divwrap-hotelpage-top">
                    <div className="divhp-hotel-name">
                      <div className="span24">
                        <div className="span25">
                          <div className="all-hostels">Hostel</div>
                        </div>
                        <div className="span26">
                          <div className="all-hostels">New to Booking.com</div>
                        </div>
                      </div>
                      <b className="a-sleepy-fox1">A Sleepy fox</b>
                    </div>
                    <div className="pshowmap2">
                      <img className="span-icon" alt="" src="../span@2x.png" />
                      <div className="b12a-kailash-colony">
                        B12A kailash colony Greater kailsh 1, Kailash Colony,
                        110048 New Delhi, India
                      </div>
                      <div className="span27">
                        <div className="excellent-location-container">
                          <span className="metro-access-txt-container">
                            <span>{`– `}</span>
                            <b className="excellent-location-">
                              {" "}
                              Excellent location - show map
                            </b>
                          </span>
                        </div>
                      </div>
                      <div className="metro-access">
                        <span className="metro-access-txt-container">
                          <span>{`– `}</span>
                          <span className="metro-access1"> Metro access</span>
                        </span>
                      </div>
                    </div>
                    <div className="formtop-book">
                      <div className="buttonhcta">
                        <div className="pseudo2" />
                        <div className="show-on-map">Reserve</div>
                      </div>
                      <img className="svg-icon16" alt="" src="../svg16.svg" />
                      <div className="buttonshare-center-btn">
                        <div className="i">
                          <div className="div602">ꀐ</div>
                        </div>
                      </div>
                      <div className="button51">
                        <div className="span22">
                          <div className="span22">
                            <img
                              className="frame-icon2"
                              alt=""
                              src="../frame10.svg"
                            />
                          </div>
                        </div>
                        <div className="we-price-match">We Price Match</div>
                      </div>
                    </div>
                  </div>
                  <div className="div603">
                    <img className="a-icon" alt="" src="../a@2x.png" />
                    <img className="a-icon1" alt="" src="../a1@2x.png" />
                    <img className="a-icon2" alt="" src="../a2@2x.png" />
                    <div className="div604">
                      <img className="a-icon3" alt="" src="../a3@2x.png" />
                      <img className="a-icon4" alt="" src="../a4@2x.png" />
                      <img className="a-icon5" alt="" src="../a5@2x.png" />
                      <img className="a-icon6" alt="" src="../a6@2x.png" />
                      <div className="a29">
                        <div className="span29">
                          <div className="span30">
                            <b className="photos">+25 photos</b>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="divreviewfloater">
                      <div className="divjs-hp-gallery-scorecard">
                        <div className="div605">
                          <div className="scored-93">
                            <div className="div606">9.3</div>
                          </div>
                          <div className="div607">
                            <div className="superb">Superb</div>
                            <div className="reviews">22 reviews</div>
                          </div>
                        </div>
                      </div>
                      <div className="div608">
                        <div className="div609">
                          <div className="div610">
                            <div className="p1">
                              <div className="silence-and-good">
                                silence and good place to work. staff behaviour
                                and cooperation is good good food
                              </div>
                            </div>
                            <div className="div611">
                              <div className="div612">
                                <div className="div613">
                                  <div className="all-hostels">Ajay</div>
                                  <div className="span31">
                                    <img
                                      className="bb04ae7a09e77da6742be779ac06cf-icon"
                                      alt=""
                                      src="../8017bb04ae7a09e77da6742be779ac06cf7fc6b0png@2x.png"
                                    />
                                    <div className="india1">India</div>
                                  </div>
                                </div>
                                <div className="div614">
                                  <img
                                    className="photojpg64-icon"
                                    alt=""
                                    src="../photojpg64@2x.png"
                                  />
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                        <div className="div615" />
                        <div className="div616" />
                      </div>
                      <div className="div617">
                        <b className="free-wifi">Free WiFi</b>
                        <div className="scored-10">
                          <div className="div618">10</div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="div619">
                <div className="div620">
                  <div className="div621">
                    <div className="divproperty-description-conte">
                      <div className="p2">
                        <div className="youre-eligible-for-container">
                          <span className="metro-access-txt-container">
                            <span>{`You're eligible for a Genius discount at A Sleepy fox! To save at this property, all you have to do is `}</span>
                            <span className="enter-the-dates">sign in</span>
                            <span>.</span>
                          </span>
                        </div>
                      </div>
                      <div className="situated-in-new">
                        Situated in New Delhi, within 4.1 miles of Humayun's
                        Tomb and 4.4 miles of Tughlaqabad Fort, A Sleepy fox
                        features accommodation with a shared lounge and free
                        WiFi throughout the property as well as free private
                        parking for guests who drive. The property is located
                        4.9 miles from Lodhi Gardens, 5 miles from Pragati
                        Maidan and 5.1 miles from Qutub Minar. Gandhi Smriti is
                        5.1 miles away and India Gate is 5.2 miles from the
                        hostel.
                      </div>
                      <div className="at-the-hostel">
                        At the hostel, the rooms come with air conditioning and
                        a private bathroom.
                      </div>
                      <div className="an-asian-breakfast">
                        An Asian breakfast is available each morning at A Sleepy
                        fox.
                      </div>
                      <div className="rashtrapati-bhavan-is">
                        Rashtrapati Bhavan is 6.8 miles from the accommodation,
                        while Jantar Mantar is 6.8 miles from the property. The
                        nearest airport is Delhi International Airport, 13 miles
                        from A Sleepy fox.
                      </div>
                    </div>
                    <div className="span32">
                      <b className="a-sleepy-fox2">
                        A Sleepy fox has been welcoming HostelHoppers since 22
                        Feb 2023.
                      </b>
                    </div>
                    <div className="distance-in-property">
                      Distance in property description is calculated using ©
                      OpenStreetMap
                    </div>
                  </div>
                  <div className="div622">
                    <b className="most-popular-facilities">
                      Most popular facilities
                    </b>
                    <div className="div623">
                      <div className="div624">
                        <div className="svg11">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame11.svg"
                          />
                        </div>
                        <div className="free-parking">Free WiFi</div>
                      </div>
                      <div className="div625">
                        <div className="svg11">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame12.svg"
                          />
                        </div>
                        <div className="free-parking">Free parking</div>
                      </div>
                      <div className="div626">
                        <div className="svg11">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame13.svg"
                          />
                        </div>
                        <div className="free-parking">Breakfast</div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="div627">
                  <b className="property-highlights">Property highlights</b>
                  <div className="p3">
                    <img className="svg-icon17" alt="" src="../svg17.svg" />
                    <div className="top-location-highly">
                      Top location: Highly rated by recent guests (9.7)
                    </div>
                  </div>
                  <div className="div628">
                    <b className="breakfast-info">Breakfast info</b>
                    <div className="asian">Asian</div>
                  </div>
                  <div className="p4">
                    <img className="svg-icon18" alt="" src="../svg18.svg" />
                    <div className="free-private-parking">
                      Free private parking available on-site
                    </div>
                  </div>
                  <div className="div629">
                    <b className="genius-discount">Genius discount</b>
                    <div className="span33">
                      <div className="youre-eligible-for-container2">
                        <span className="metro-access-txt-container">
                          <span>{`You're eligible for a discount with our loyalty programme when you `}</span>
                          <span className="sign-in2"> sign in</span>
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="button52">
                    <div className="pseudo3" />
                    <div className="reserve1">Reserve</div>
                  </div>
                </div>
              </div>
              <div className="hr" />
              <div className="divhp-availability-style-chan">
                <div className="div630">
                  <b className="availability">Availability</b>
                  <div className="button53">
                    <div className="span22">
                      <div className="span22">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame14.svg"
                        />
                      </div>
                    </div>
                    <div className="we-price-match">We Price Match</div>
                  </div>
                </div>
                <div className="div631">
                  <img className="svg-icon19" alt="" src="../svg19.svg" />
                  <div className="select-dates-to">
                    Select dates to see this property's availability and prices
                  </div>
                </div>
                <div className="div632">
                  <div className="div633">
                    <div className="div634">
                      <div className="div635">
                        <div className="div636">
                          <div className="div637">
                            <div className="we-price-match">Check-in</div>
                          </div>
                        </div>
                        <div className="div638">
                          <div className="div639">
                            <div className="div640" />
                            <div className="div641">놀</div>
                          </div>
                          <div className="div642">
                            <div className="div643" />
                            <div className="div644">놀</div>
                          </div>
                        </div>
                        <img className="svg-icon20" alt="" src="../svg20.svg" />
                      </div>
                      <div className="pseudo4" />
                    </div>
                    <div className="div645">
                      <div className="div635">
                        <div className="div647">
                          <div className="div648">
                            <div className="we-price-match">Check-out</div>
                          </div>
                        </div>
                        <div className="div649">
                          <div className="div650">
                            <div className="div651" />
                            <div className="div652">놀</div>
                          </div>
                          <div className="div653">
                            <div className="div654" />
                            <div className="div655">놀</div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="labelxp-guests-toggle-wrapp">
                    <img
                      className="b2e5f2aa32b71ca0fc66aa671e4e95-icon"
                      alt=""
                      src="../b2e5f2aa32b71ca0fc66aa671e4e958bcd69b7d0svg.svg"
                    />
                    <div className="span35">
                      <div className="adults">2 adults</div>
                      <div className="span36">
                        <div className="we-price-match">· 0 children</div>
                      </div>
                      <div className="span37">
                        <div className="we-price-match">· 1 room</div>
                      </div>
                    </div>
                    <img
                      className="labelxp-guests-toggle-icon"
                      alt=""
                      src="../labelxp--guests--toggle.svg"
                    />
                  </div>
                  <div className="button54">
                    <div className="we-price-match">Search</div>
                  </div>
                </div>
                <div className="section">
                  <div className="div656">
                    <div className="div657">
                      <b className="room-type">Room type</b>
                    </div>
                    <div className="div658">
                      <b className="sleeps">Sleeps</b>
                    </div>
                    <div className="div659" />
                  </div>
                  <div className="div660">
                    <div className="div661">
                      <b className="deluxe-double-or">
                        Deluxe Double or Twin Room
                      </b>
                      <div className="div662">
                        <div className="large-double-bed">
                          1 large double bed
                        </div>
                        <div className="svg15">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame15.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div663">
                      <div className="span38">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame16.svg"
                          />
                        </div>
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame16.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="span39">
                      <div className="pseudo5" />
                      <div className="show-prices">Show prices</div>
                    </div>
                  </div>
                  <div className="div664">
                    <div className="div661">
                      <b className="deluxe-double-room">
                        Deluxe Double Room with Balcony
                      </b>
                      <div className="div662">
                        <div className="large-double-bed">
                          1 large double bed
                        </div>
                        <div className="svg15">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame15.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div663">
                      <div className="span38">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame16.svg"
                          />
                        </div>
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame16.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="span39">
                      <div className="pseudo5" />
                      <div className="show-prices">Show prices</div>
                    </div>
                  </div>
                  <div className="div668">
                    <div className="div661">
                      <b className="bed-in-8-bed">
                        Bed in 8-Bed Mixed Dormitory Room
                      </b>
                      <div className="div670">
                        <div className="large-double-bed">1 bunk bed</div>
                        <div className="svg21">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame21.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div663">
                      <div className="svg22">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame16.svg"
                        />
                      </div>
                    </div>
                    <div className="span39">
                      <div className="pseudo5" />
                      <div className="show-prices">Show prices</div>
                    </div>
                  </div>
                  <div className="div672">
                    <div className="div661">
                      <b className="bed-in-8-bed">
                        Bed in 6-Bed Mixed Dormitory Room
                      </b>
                      <div className="div670">
                        <div className="large-double-bed">1 bunk bed</div>
                        <div className="svg21">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame21.svg"
                          />
                        </div>
                      </div>
                    </div>
                    <div className="div663">
                      <div className="svg22">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame16.svg"
                        />
                      </div>
                    </div>
                    <div className="span39">
                      <div className="pseudo5" />
                      <div className="show-prices">Show prices</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="div676">
                <img className="svg-icon21" alt="" src="../svg21.svg" />
                <div className="p5">
                  <div className="no-credit-card-container">
                    <b>No credit card needed to book.</b>
                    <span>
                      {" "}
                      We'll send you an email confirming your reservation.
                    </span>
                  </div>
                </div>
              </div>
              <div className="divguest-featured-reviews-ho">
                <div className="button55">
                  <div className="pseudo9" />
                  <div className="show-on-map">See availability</div>
                </div>
                <div className="div677">
                  <div className="div678">
                    <b className="guest-reviews">Guest reviews</b>
                    <div className="div679">
                      <div className="div680">
                        <div className="scored-931">
                          <div className="div606">9.3</div>
                        </div>
                        <div className="div682">
                          <div className="superb1">Superb</div>
                          <div className="span44">
                            <div className="superb1">· 22 reviews</div>
                          </div>
                        </div>
                      </div>
                      <div className="read-all-reviews">Read all reviews</div>
                    </div>
                  </div>
                  <div className="div683">
                    <b className="categories">Categories:</b>
                    <div className="div684">
                      <div className="div685">
                        <div className="div686">
                          <div className="span45">
                            <div className="we-price-match">Staff</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame25.svg"
                              />
                            </div>
                          </div>
                          <div className="we-price-match">9.7</div>
                        </div>
                        <div className="div688">
                          <div className="span46" />
                        </div>
                      </div>
                      <div className="div689">
                        <div className="div690">
                          <div className="span47">
                            <div className="we-price-match">Facilities</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame26.svg"
                              />
                            </div>
                          </div>
                          <div className="we-price-match">9.2</div>
                        </div>
                        <div className="div688">
                          <div className="span48" />
                        </div>
                      </div>
                      <div className="div693">
                        <div className="div694">
                          <div className="span49">
                            <div className="we-price-match">Cleanliness</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame27.svg"
                              />
                            </div>
                          </div>
                          <div className="div695">9.2</div>
                        </div>
                        <div className="div688">
                          <div className="span48" />
                        </div>
                      </div>
                      <div className="div697">
                        <div className="div698">
                          <div className="span51">
                            <div className="we-price-match">Comfort</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame28.svg"
                              />
                            </div>
                          </div>
                          <div className="we-price-match">9.5</div>
                        </div>
                        <div className="div688">
                          <div className="span52" />
                        </div>
                      </div>
                      <div className="div701">
                        <div className="div702">
                          <div className="span53">
                            <div className="we-price-match">
                              Value for money
                            </div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame29.svg"
                              />
                            </div>
                          </div>
                          <div className="we-price-match">9.5</div>
                        </div>
                        <div className="div688">
                          <div className="span52" />
                        </div>
                      </div>
                      <div className="div705">
                        <div className="div706">
                          <div className="span55">
                            <div className="we-price-match">Location</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame30.svg"
                              />
                            </div>
                          </div>
                          <div className="we-price-match">9.7</div>
                        </div>
                        <div className="div688">
                          <div className="span46" />
                        </div>
                      </div>
                      <div className="div709">
                        <div className="div710">
                          <div className="span57">
                            <div className="we-price-match">Free WiFi</div>
                            <div className="span22">
                              <img
                                className="frame-icon25"
                                alt=""
                                src="../frame31.svg"
                              />
                            </div>
                          </div>
                          <div className="div695">10</div>
                        </div>
                        <div className="div688">
                          <div className="span58" />
                        </div>
                      </div>
                      <div className="div713">
                        <div className="span22">
                          <img
                            className="frame-icon25"
                            alt=""
                            src="../frame30.svg"
                          />
                        </div>
                        <div className="all-hostels">
                          High score for New Delhi
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="div714">
                    <b className="select-topics-to">
                      Select topics to read reviews:
                    </b>
                    <div className="div715">
                      <div className="span59">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame33.svg"
                          />
                        </div>
                        <div className="div716">
                          <div className="we-price-match">Clean</div>
                        </div>
                      </div>
                      <div className="span60">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame34.svg"
                          />
                        </div>
                        <div className="div717">
                          <div className="we-price-match">Bathroom</div>
                        </div>
                      </div>
                      <div className="span61">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame35.svg"
                          />
                        </div>
                        <div className="span22">
                          <div className="we-price-match">Bed</div>
                        </div>
                      </div>
                      <div className="span60">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame36.svg"
                          />
                        </div>
                        <div className="div719">
                          <div className="we-price-match">Breakfast</div>
                        </div>
                      </div>
                      <div className="span60">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame37.svg"
                          />
                        </div>
                        <div className="div720">
                          <div className="we-price-match">Location</div>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="div721">
                    <div className="div722">
                      <b className="see-what-guests">
                        See what guests loved the most:
                      </b>
                      <div className="div723">
                        <div className="ul3">
                          <div className="div724">
                            <div className="div725">
                              <div className="div726">
                                <div className="div727">
                                  <b className="no-credit-card-container">
                                    Ajay
                                  </b>
                                  <div className="div728">
                                    <img
                                      className="image-icon"
                                      alt=""
                                      src="../image@2x.png"
                                    />
                                    <div className="india2">India</div>
                                  </div>
                                </div>
                                <div className="div729">
                                  <img
                                    className="photojpg-icon"
                                    alt=""
                                    src="../photojpg@2x.png"
                                  />
                                </div>
                              </div>
                              <div className="div730">
                                <div className="silence-and-good-container">
                                  <span className="metro-access-txt-container">
                                    <p className="staff-behaviour-and">{`“silence and good place to work. `}</p>
                                    <p className="staff-behaviour-and">
                                      staff behaviour and cooperation is good
                                    </p>
                                    <p className="good-food">good food”</p>
                                  </span>
                                </div>
                              </div>
                              <div className="read-more1">Read more</div>
                            </div>
                          </div>
                          <div className="div731">
                            <div className="div732">
                              <div className="div726">
                                <div className="div727">
                                  <b className="no-credit-card-container">
                                    Pathiraja
                                  </b>
                                  <div className="div728">
                                    <img
                                      className="image-icon"
                                      alt=""
                                      src="../image1@2x.png"
                                    />
                                    <div className="sri-lanka">Sri Lanka</div>
                                  </div>
                                </div>
                                <div className="div729">
                                  <img
                                    className="photojpg-icon"
                                    alt=""
                                    src="../afdzucqprap0k4w-ntdv7wxl7vbgbhg9tmburpbwo9rrxws96c@2x.png"
                                  />
                                </div>
                              </div>
                              <div className="div737">
                                <div className="it-was-not">{`“It was not such a big hotel as I expected,But they had managed the little space well.Staff is so friendly & supportive.They are very negotiable & can customize your stay.Security wise also good.They have enjoyable parties.it was a good stay.”`}</div>
                              </div>
                              <div className="read-more2">Read more</div>
                            </div>
                          </div>
                          <div className="div738">
                            <div className="div732">
                              <div className="div726">
                                <div className="div727">
                                  <b className="no-credit-card-container">
                                    Upender
                                  </b>
                                  <div className="div728">
                                    <img
                                      className="image-icon"
                                      alt=""
                                      src="../image2@2x.png"
                                    />
                                    <div className="india2">India</div>
                                  </div>
                                </div>
                                <div className="div729">
                                  <img
                                    className="photojpg-icon"
                                    alt=""
                                    src="../avaupng@2x.png"
                                  />
                                </div>
                              </div>
                              <div className="div737">
                                <div className="they-were-extremely">
                                  “They were extremely accommodating and allowed
                                  us to check in early at like 10am. We got to
                                  hostel super early and I didn’t wanna wait. So
                                  this was a big plus. The sevice was
                                  exceptional as well. Would definitely send a
                                  friend there.”
                                </div>
                              </div>
                              <div className="read-more2">Read more</div>
                            </div>
                          </div>
                        </div>
                        <div className="next">
                          <div className="span22">
                            <div className="span22">
                              <img
                                className="frame-icon2"
                                alt=""
                                src="../frame38.svg"
                              />
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                    <div className="button56">
                      <div className="pseudo10" />
                      <div className="show-on-map">Read all reviews</div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="section1">
                <div className="div745">
                  <div className="svg39">
                    <img className="frame-icon39" alt="" src="../frame39.svg" />
                  </div>
                  <div className="header">
                    <b className="no-credit-card-container">
                      Property questions and answers
                    </b>
                    <div className="no-credit-card-container">
                      Send a question to the property to find out more.
                    </div>
                  </div>
                  <div className="button57">
                    <div className="pseudo11" />
                    <div className="show-on-map">Ask a question</div>
                  </div>
                </div>
              </div>
              <div className="div746">
                <div className="div747">
                  <b className="property-surroundings">
                    Property surroundings *
                  </b>
                  <div className="button58">
                    <div className="pseudo9" />
                    <div className="show-on-map">See availability</div>
                  </div>
                </div>
                <div className="div748">
                  <div className="excellent-location-1">
                    Excellent location - show map
                  </div>
                  <div className="div749">
                    <div className="div750">
                      <div className="div751">
                        <div className="span22">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame40.svg"
                          />
                        </div>
                        <b className="whats-nearby">What's nearby</b>
                      </div>
                      <div className="ul4">
                        <div className="div752">
                          <div className="no-credit-card-container">
                            Kailash Colony Park
                          </div>
                          <div className="ft">550 ft</div>
                        </div>
                        <div className="div753">
                          <div className="no-credit-card-container">
                            E block Basket Ball park
                          </div>
                          <div className="ft">600 ft</div>
                        </div>
                        <div className="div754">
                          <div className="no-credit-card-container">
                            Children's Park
                          </div>
                          <div className="ft">1,100 ft</div>
                        </div>
                        <div className="div755">
                          <div className="no-credit-card-container">
                            R-block park
                          </div>
                          <div className="ft">1,750 ft</div>
                        </div>
                        <div className="div756">
                          <div className="no-credit-card-container">
                            d block park
                          </div>
                          <div className="ft4">2,450 ft</div>
                        </div>
                        <div className="div757">
                          <div className="no-credit-card-container">
                            Sant nagar park
                          </div>
                          <div className="ft">2,800 ft</div>
                        </div>
                        <div className="div758">
                          <div className="no-credit-card-container">
                            Kailash Colony Park
                          </div>
                          <div className="ft">2,900 ft</div>
                        </div>
                        <div className="div759">
                          <div className="no-credit-card-container">
                            Sapna Park
                          </div>
                          <div className="ft">2,950 ft</div>
                        </div>
                        <div className="div760">
                          <div className="no-credit-card-container">
                            N Block Park
                          </div>
                          <div className="ft">3,100 ft</div>
                        </div>
                        <div className="div761">
                          <div className="no-credit-card-container">
                            Family Park
                          </div>
                          <div className="ft">0.7 mi</div>
                        </div>
                      </div>
                    </div>
                    <div className="div762">
                      <div className="div763">
                        <div className="span22">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame41.svg"
                          />
                        </div>
                        <b className="whats-nearby">{`Restaurants & cafes`}</b>
                      </div>
                      <div className="ul5">
                        <div className="div764">
                          <div className="div765">
                            <div className="span65">
                              <div className="cafebar">Cafe/bar</div>
                              <div className="pseudo13" />
                            </div>
                            <div className="milkshake-and-co">
                              Milkshake and Co.
                            </div>
                          </div>
                          <div className="no-credit-card-container">250 ft</div>
                        </div>
                        <div className="div766">
                          <div className="div767">
                            <div className="span66">
                              <div className="cafebar">Restaurant</div>
                              <div className="pseudo14" />
                            </div>
                            <div className="uncultured-cafe-sky">
                              Uncultured Cafe Sky Culture
                            </div>
                          </div>
                          <div className="ft10">300 ft</div>
                        </div>
                        <div className="div766">
                          <div className="div769">
                            <div className="span66">
                              <div className="cafebar">Restaurant</div>
                              <div className="pseudo14" />
                            </div>
                            <div className="juggernaut">Juggernaut</div>
                          </div>
                          <div className="ft10">550 ft</div>
                        </div>
                      </div>
                    </div>
                    <div className="div770">
                      <div className="div771">
                        <div className="span22">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame42.svg"
                          />
                        </div>
                        <b className="whats-nearby">Top attractions</b>
                      </div>
                      <div className="ul4">
                        <div className="div772">
                          <div className="no-credit-card-container">
                            Tughlaqabad Fort
                          </div>
                          <div className="ft">2.9 mi</div>
                        </div>
                        <div className="div773">
                          <div className="no-credit-card-container">
                            Humayun's Tomb
                          </div>
                          <div className="ft">3 mi</div>
                        </div>
                        <div className="div774">
                          <div className="no-credit-card-container">
                            Lodhi Gardens
                          </div>
                          <div className="ft">3.2 mi</div>
                        </div>
                        <div className="div775">
                          <div className="no-credit-card-container">
                            Gandhi Smriti
                          </div>
                          <div className="ft">3.9 mi</div>
                        </div>
                        <div className="div776">
                          <div className="no-credit-card-container">
                            Qutub Minar
                          </div>
                          <div className="ft">3.9 mi</div>
                        </div>
                        <div className="div777">
                          <div className="no-credit-card-container">
                            Purana Qila (old fort)
                          </div>
                          <div className="ft">4 mi</div>
                        </div>
                        <div className="div778">
                          <div className="no-credit-card-container">
                            India Gate
                          </div>
                          <div className="ft">4.3 mi</div>
                        </div>
                        <div className="div779">
                          <div className="no-credit-card-container">
                            National Gandhi Museum
                          </div>
                          <div className="ft">6 mi</div>
                        </div>
                        <div className="div780">
                          <div className="no-credit-card-container">
                            Rāj Ghāt
                          </div>
                          <div className="ft">7 mi</div>
                        </div>
                        <div className="div780">
                          <div className="no-credit-card-container">
                            Red Fort
                          </div>
                          <div className="ft">7 mi</div>
                        </div>
                      </div>
                    </div>
                    <div className="div782">
                      <div className="div783">
                        <div className="span22">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame43.svg"
                          />
                        </div>
                        <b className="whats-nearby">Public transport</b>
                      </div>
                      <div className="ul5">
                        <div className="div784">
                          <div className="div785">
                            <div className="span68">
                              <div className="cafebar">Metro</div>
                              <div className="pseudo16" />
                            </div>
                            <div className="milkshake-and-co">
                              Kailash Colony
                            </div>
                          </div>
                          <div className="no-credit-card-container">
                            1,250 ft
                          </div>
                        </div>
                        <div className="div784">
                          <div className="div787">
                            <div className="span68">
                              <div className="cafebar">Metro</div>
                              <div className="pseudo16" />
                            </div>
                            <div className="milkshake-and-co">Nehru Place</div>
                          </div>
                          <div className="no-credit-card-container">
                            3,050 ft
                          </div>
                        </div>
                        <div className="div788">
                          <div className="div789">
                            <div className="span70">
                              <div className="cafebar">Train</div>
                              <div className="pseudo18" />
                            </div>
                            <div className="juggernaut">
                              Moolchand Metro Station
                            </div>
                          </div>
                          <div className="ft10">1 mi</div>
                        </div>
                        <div className="div790">
                          <div className="div791">
                            <div className="span70">
                              <div className="cafebar">Train</div>
                              <div className="pseudo18" />
                            </div>
                            <div className="uncultured-cafe-sky">Okhla</div>
                          </div>
                          <div className="ft10">1.6 mi</div>
                        </div>
                        <div className="div792">
                          <div className="div793">
                            <div className="span72">
                              <div className="cafebar">Bus</div>
                              <div className="pseudo20" />
                            </div>
                            <div className="maharana-pratap-inter">
                              Maharana Pratap Inter State Bus Terminus
                            </div>
                          </div>
                          <div className="ft10">8 mi</div>
                        </div>
                      </div>
                    </div>
                    <div className="div794">
                      <div className="div795">
                        <div className="span22">
                          <img
                            className="frame-icon4"
                            alt=""
                            src="../frame44.svg"
                          />
                        </div>
                        <b className="whats-nearby">Closest airports</b>
                      </div>
                      <div className="div796">
                        <div className="no-credit-card-container">
                          Delhi International Airport
                        </div>
                        <div className="ft4">9 mi</div>
                      </div>
                    </div>
                  </div>
                  <div className="all-distances-are">
                    * All distances are measured in straight lines. Actual
                    travel distances may vary.
                  </div>
                  <div className="div797">
                    <div className="no-credit-card-container">
                      Missing some information?
                    </div>
                    <div className="div798">
                      <b className="no-credit-card-container">Yes</b>
                      <div className="div799">
                        <b className="no-credit-card-container">/</b>
                        <b className="no">No</b>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="sectionhp-facilities-box">
                <div className="divhotelfacilities">
                  <b className="facilities-of-a">Facilities of A Sleepy fox</b>
                  <div className="buttoncta-facilities">
                    <div className="pseudo21" />
                    <div className="show-on-map">See availability</div>
                  </div>
                </div>
                <div className="div800">
                  <b className="most-popular-facilities">
                    Most popular facilities
                  </b>
                  <div className="div801">
                    <div className="div624">
                      <div className="svg11">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame45.svg"
                        />
                      </div>
                      <div className="free-parking">Free WiFi</div>
                    </div>
                    <div className="div625">
                      <div className="svg11">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame46.svg"
                        />
                      </div>
                      <div className="free-parking">Free parking</div>
                    </div>
                    <div className="div626">
                      <div className="svg11">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame47.svg"
                        />
                      </div>
                      <div className="free-parking">Breakfast</div>
                    </div>
                  </div>
                </div>
                <div className="div805">
                  <div className="div806">
                    <div className="div807">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame48.svg"
                        />
                      </div>
                      <b className="whats-nearby">Bathroom</b>
                    </div>
                    <div className="div808">
                      <div className="svg49">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame49.svg"
                        />
                      </div>
                      <div className="free-parking">Private bathroom</div>
                    </div>
                  </div>
                  <div className="div809">
                    <div className="div810">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame50.svg"
                        />
                      </div>
                      <b className="whats-nearby">Bedroom</b>
                    </div>
                    <div className="div811">
                      <div className="svg49">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame51.svg"
                        />
                      </div>
                      <div className="free-parking">Linen</div>
                    </div>
                  </div>
                  <div className="div812">
                    <div className="div813">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame52.svg"
                        />
                      </div>
                      <b className="whats-nearby">Activities</b>
                    </div>
                    <div className="ul8">
                      <div className="div814">
                        <div className="span22">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame53.svg"
                          />
                        </div>
                        <div className="div815">
                          <div className="no-credit-card-container">
                            Walking tours
                          </div>
                          <div className="span73">
                            <div className="all-hostels">Additional charge</div>
                          </div>
                        </div>
                      </div>
                      <div className="div816">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame54.svg"
                          />
                        </div>
                        <div className="free-parking">Movie nights</div>
                      </div>
                      <div className="div817">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame55.svg"
                          />
                        </div>
                        <div className="free-parking">
                          Evening entertainment
                        </div>
                      </div>
                      <div className="div817">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame56.svg"
                          />
                        </div>
                        <div className="free-parking">Games room</div>
                      </div>
                    </div>
                  </div>
                  <div className="div819">
                    <div className="div820">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame57.svg"
                        />
                      </div>
                      <b className="whats-nearby">Internet</b>
                    </div>
                    <div className="wifi-is-available">
                      WiFi is available in all areas and is free of charge.
                    </div>
                  </div>
                  <div className="div821">
                    <div className="div822">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame58.svg"
                        />
                      </div>
                      <b className="whats-nearby">Parking</b>
                    </div>
                    <div className="free-private-parking1">
                      Free private parking is possible on site (reservation is
                      needed).
                    </div>
                  </div>
                  <div className="div823">
                    <div className="div824">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame59.svg"
                        />
                      </div>
                      <b className="whats-nearby">Services</b>
                    </div>
                    <div className="div825">
                      <div className="svg49">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame60.svg"
                        />
                      </div>
                      <div className="free-parking">Shared lounge/TV area</div>
                    </div>
                  </div>
                  <div className="div826">
                    <div className="div827">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame61.svg"
                        />
                      </div>
                      <b className="whats-nearby">
                        Entertainment and family services
                      </b>
                    </div>
                    <div className="div828">
                      <div className="svg49">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame62.svg"
                        />
                      </div>
                      <div className="free-parking">Board games/puzzles</div>
                    </div>
                  </div>
                  <div className="div829">
                    <div className="div830">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame63.svg"
                        />
                      </div>
                      <b className="whats-nearby">General</b>
                    </div>
                    <div className="ul9">
                      <div className="div831">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame64.svg"
                          />
                        </div>
                        <div className="free-parking">Fan</div>
                      </div>
                      <div className="div831">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame65.svg"
                          />
                        </div>
                        <div className="free-parking">Iron</div>
                      </div>
                      <div className="div833">
                        <div className="svg49">
                          <img
                            className="frame-icon2"
                            alt=""
                            src="../frame66.svg"
                          />
                        </div>
                        <div className="free-parking">Air conditioning</div>
                      </div>
                    </div>
                  </div>
                  <div className="div834">
                    <div className="div835">
                      <div className="span22">
                        <img
                          className="frame-icon4"
                          alt=""
                          src="../frame67.svg"
                        />
                      </div>
                      <b className="whats-nearby">Languages spoken</b>
                    </div>
                    <div className="div836">
                      <div className="svg49">
                        <img
                          className="frame-icon2"
                          alt=""
                          src="../frame68.svg"
                        />
                      </div>
                      <div className="free-parking">English</div>
                    </div>
                  </div>
                </div>
                <div className="div837">
                  <div className="missing-some-information1">
                    Missing some information?
                  </div>
                  <div className="div838">
                    <b className="no-credit-card-container">Yes</b>
                    <div className="div799">
                      <b className="no-credit-card-container">/</b>
                      <b className="no">No</b>
                    </div>
                  </div>
                </div>
              </div>
              <div className="divhp-policies-box">
                <div className="div840">
                  <div className="h2">
                    <b className="house-rules2">House rules</b>
                    <div className="buttoncta-policies">
                      <div className="pseudo21" />
                      <div className="show-on-map">See availability</div>
                    </div>
                  </div>
                  <div className="a-sleepy-fox3">
                    A Sleepy fox takes special requests - add in the next step!
                  </div>
                </div>
                <div className="divhotelpoliciesinc">
                  <div className="divcheckin-policy">
                    <div className="p6">
                      <img className="svg-icon22" alt="" src="../svg22.svg" />
                      <div className="check-in1">Check-in</div>
                    </div>
                    <div className="span74">
                      <div className="span75">
                        <div className="span76" />
                      </div>
                      <div className="div841">00:00</div>
                      <div className="div842">00:00</div>
                      <div className="span77">
                        <div className="no-credit-card-container">
                          00:00 - 00:00
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="divcheckout-policy">
                    <div className="p6">
                      <img className="svg-icon22" alt="" src="../svg23.svg" />
                      <div className="check-out1">Check-out</div>
                    </div>
                    <div className="span74">
                      <div className="span75">
                        <div className="span80" />
                      </div>
                      <div className="div844">11:00</div>
                      <div className="span81">
                        <div className="no-credit-card-container">
                          Until 11:00 hours
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="divcancellation-policy">
                    <div className="p8">
                      <img className="svg-icon24" alt="" src="../svg24.svg" />
                      <div className="span82">
                        <div className="cancellation-prepayment">
                          <p className="staff-behaviour-and">Cancellation/</p>
                          <p className="good-food">prepayment</p>
                        </div>
                      </div>
                    </div>
                    <div className="p9">
                      <div className="cancellation-and-prepayment-container">
                        <span className="metro-access-txt-container">
                          <span>{`Cancellation and prepayment policies vary according to accommodation type. Please `}</span>
                          <b className="enter-the-dates">
                            enter the dates of your stay
                          </b>
                          <span>
                            {" "}
                            and check the conditions of your required room.
                          </span>
                        </span>
                      </div>
                    </div>
                  </div>
                  <div className="divsmp-children-policy">
                    <div className="p10">
                      <img className="svg-icon25" alt="" src="../svg25.svg" />
                      <div className="children-and-beds">Children and beds</div>
                    </div>
                    <div className="div845">
                      <div className="div846">
                        <b className="child-policies">Child policies</b>
                        <div className="children-of-any">
                          Children of any age are welcome.
                        </div>
                        <div className="to-see-correct">
                          To see correct prices and occupancy information,
                          please add the number of children in your group and
                          their ages to your search.
                        </div>
                      </div>
                      <b className="cot-and-extra">
                        Cot and extra bed policies
                      </b>
                      <div className="div847">
                        <div className="div848">
                          <div className="adult">Adult</div>
                          <div className="span83">
                            <div className="we-price-match">(18+ years)</div>
                          </div>
                        </div>
                        <div className="div849">
                          <div className="span84">
                            <div className="span22">
                              <img
                                className="frame-icon4"
                                alt=""
                                src="../frame69.svg"
                              />
                            </div>
                            <div className="no-credit-card-container">
                              Extra bed upon request
                            </div>
                          </div>
                          <div className="rs-500-per">
                            Rs. 500 per person, per night
                          </div>
                        </div>
                      </div>
                      <div className="div850">
                        <div className="prices-for-extra">
                          Prices for extra beds are not included in the total
                          price, and will have to be paid for separately during
                          your stay.
                        </div>
                        <div className="the-number-of">
                          The number of extra beds allowed is dependent on the
                          option you choose. Please check your selected option
                          for more information.
                        </div>
                        <div className="there-are-no">
                          There are no cots available at this property.
                        </div>
                        <div className="all-extra-beds">
                          All extra beds are subject to availability.
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="divage-restriction-policy">
                    <div className="p10">
                      <img className="svg-icon25" alt="" src="../svg26.svg" />
                      <div className="no-age-restriction">
                        No age restriction
                      </div>
                    </div>
                    <div className="there-is-no">
                      There is no age requirement for check-in
                    </div>
                  </div>
                  <div className="div851">
                    <div className="p10">
                      <img className="svg-icon25" alt="" src="../svg27.svg" />
                      <div className="pets">Pets</div>
                    </div>
                    <div className="pets-are-not">Pets are not allowed.</div>
                  </div>
                  <div className="div852">
                    <div className="p10">
                      <img className="svg-icon25" alt="" src="../svg28.svg" />
                      <div className="cash-only">Cash only</div>
                    </div>
                    <div className="this-property-only">
                      This property only accepts cash payments.
                    </div>
                  </div>
                </div>
              </div>
              <div className="divhp-small-print">
                <div className="h21">
                  <div className="buttoncta-fine-print">
                    <div className="pseudo21" />
                    <div className="show-on-map">See availability</div>
                  </div>
                  <b className="the-fine-print">The fine print</b>
                </div>
                <div className="div853">
                  <div className="license-number">License number:</div>
                </div>
              </div>
              <div className="div854">
                <b className="faqs-about-a">FAQs about A Sleepy fox</b>
                <div className="accordion-control">
                  <div className="button59">
                    <b className="what-are-the">
                      What are the check-in and check-out times at A Sleepy fox?
                    </b>
                    <img className="svg-icon29" alt="" src="../svg29.svg" />
                  </div>
                  <div className="li3">
                    <div className="button60">
                      <b className="what-is-there">
                        What is there to do at A Sleepy fox?
                      </b>
                      <img className="svg-icon29" alt="" src="../svg30.svg" />
                    </div>
                  </div>
                  <div className="li4">
                    <div className="button60">
                      <b className="how-far-is">
                        How far is A Sleepy fox from the centre of New Delhi?
                      </b>
                      <img className="svg-icon29" alt="" src="../svg31.svg" />
                    </div>
                  </div>
                  <div className="li5">
                    <div className="button60">
                      <b className="how-much-does">
                        How much does it cost to stay at A Sleepy fox?
                      </b>
                      <img className="svg-icon29" alt="" src="../svg32.svg" />
                    </div>
                  </div>
                  <div className="li6">
                    <div className="button60">
                      <b className="what-kind-of">
                        What kind of breakfast is served at A Sleepy fox?
                      </b>
                      <img className="svg-icon29" alt="" src="../svg33.svg" />
                    </div>
                  </div>
                </div>
              </div>
            </div>
            <div className="div855">
              <div className="div856">
                <div className="h1">
                  <b className="travellers-who-looked">
                    Travellers who looked at A Sleepy fox ended up booking these
                    properties
                  </b>
                  <div className="show-more">Show more</div>
                </div>
                <div className="div857">
                  <div className="ul10">
                    <div className="a30">
                      <div className="div858">
                        <div className="header1">
                          <div className="h3">
                            <b className="whats-nearby">{`Tom & Jerry - Jangpura`}</b>
                          </div>
                          <div className="wifi-is-available">8.2 Very good</div>
                        </div>
                        <div className="div859">
                          <img
                            className="svg-icon34"
                            alt=""
                            src="../svg34.svg"
                          />
                          <div className="miles-from-centre">
                            3.9 miles from centre
                          </div>
                        </div>
                      </div>
                      <div className="div860">
                        <img
                          className="jpg-icon2"
                          alt=""
                          src="../427997958jpg@2x.png"
                        />
                      </div>
                    </div>
                    <div className="a31">
                      <div className="div858">
                        <div className="header1">
                          <div className="h31">
                            <b className="whats-nearby">Hotel Way Point</b>
                          </div>
                          <div className="wifi-is-available">
                            10 Exceptional
                          </div>
                        </div>
                        <div className="div859">
                          <img
                            className="svg-icon34"
                            alt=""
                            src="../svg35.svg"
                          />
                          <div className="miles-from-centre">
                            8.2 miles from centre
                          </div>
                        </div>
                      </div>
                      <div className="div860">
                        <img
                          className="jpg-icon2"
                          alt=""
                          src="../426462361jpg@2x.png"
                        />
                      </div>
                    </div>
                    <div className="a32">
                      <div className="div864">
                        <div className="header1">
                          <div className="h32">
                            <b className="hotel-rosewood-near">
                              Hotel Rosewood Near Delhi Airport
                            </b>
                          </div>
                          <div className="wifi-is-available">8 Very good</div>
                        </div>
                        <div className="div865">
                          <div className="div866">
                            <img
                              className="svg-icon34"
                              alt=""
                              src="../svg36.svg"
                            />
                            <div className="miles-from-centre">
                              9.2 miles from centre
                            </div>
                          </div>
                          <div className="div866">
                            <img
                              className="svg-icon34"
                              alt=""
                              src="../svg37.svg"
                            />
                            <div className="miles-from-centre">
                              Travel Sustainable property
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="div860">
                        <img
                          className="jpg-icon2"
                          alt=""
                          src="../426510973jpg@2x.png"
                        />
                      </div>
                    </div>
                    <div className="a33">
                      <div className="div864">
                        <div className="header1">
                          <div className="h32">
                            <b className="hotel-qotel-chattarpur">
                              Hotel Qotel chattarpur New Delhi - Couple Friendly
                            </b>
                          </div>
                          <div className="wifi-is-available">
                            5.7 Review score
                          </div>
                        </div>
                        <div className="div865">
                          <div className="div866">
                            <img
                              className="svg-icon34"
                              alt=""
                              src="../svg38.svg"
                            />
                            <div className="miles-from-centre">
                              9.8 miles from centre
                            </div>
                          </div>
                          <div className="div866">
                            <img
                              className="svg-icon34"
                              alt=""
                              src="../svg39.svg"
                            />
                            <div className="miles-from-centre">
                              Travel Sustainable property
                            </div>
                          </div>
                        </div>
                      </div>
                      <div className="div860">
                        <img
                          className="jpg-icon2"
                          alt=""
                          src="../425802129jpg@2x.png"
                        />
                      </div>
                    </div>
                  </div>
                  <div className="button64">
                    <img className="svg-icon40" alt="" src="../svg40.svg" />
                  </div>
                </div>
              </div>
            </div>
            <div className="div874">
              <div className="the-best-of">The best of New Delhi</div>
              <div className="click-here-to">
                Click here to see more properties near popular landmarks in New
                Delhi
              </div>
            </div>
          </div>
        </div>
        <div className="exclude">
          <div className="divfooter-menu-track">
            <div className="div875">
              <div className="div876">
                <div className="save-time-save">Save time, save money!</div>
                <div className="sign-up-and">
                  Sign up and we'll send the best deals to you
                </div>
                <div className="div877">
                  <div className="inputnewsletter-to">
                    <div className="divplaceholder2">
                      <div className="your-email-address">
                        Your email address
                      </div>
                    </div>
                  </div>
                  <div className="buttonnewsletter-button-foote">
                    <div className="subscribe">Subscribe</div>
                  </div>
                </div>
              </div>
            </div>
            <div className="divbooking-footer">
              <div className="divfooter-top-menu">
                <div className="div878">
                  <div className="a34">
                    <div className="list-your-property">List your property</div>
                  </div>
                </div>
                <div className="ul11">
                  <div className="li7">
                    <b className="mobile-version">Mobile version</b>
                  </div>
                  <div className="li8">
                    <b className="your-account">Your account</b>
                  </div>
                  <div className="li9">
                    <b className="make-changes-to">
                      Make changes to your booking online
                    </b>
                  </div>
                  <div className="li10">
                    <b className="customer-service-help">
                      Customer Service help
                    </b>
                  </div>
                  <div className="li11">
                    <b className="become-an-affiliate">Become an affiliate</b>
                  </div>
                  <div className="li12">
                    <b className="bookingcom-for-business">
                      Booking.com for Business
                    </b>
                  </div>
                </div>
              </div>
              <div className="divfooter-links">
                <div className="ul12">
                  <div className="countries">Countries</div>
                  <div className="regions">Regions</div>
                  <div className="cities">Cities</div>
                  <div className="districts">Districts</div>
                  <div className="airports">Airports</div>
                  <div className="hotels">Hotels</div>
                  <div className="places-of-interest">Places of interest</div>
                </div>
                <div className="ul13">
                  <div className="homes">Homes</div>
                  <div className="apartments">Apartments</div>
                  <div className="resorts">Resorts</div>
                  <div className="villas">Villas</div>
                  <div className="hostels4">Hostels</div>
                  <div className="bbs">{`B&Bs`}</div>
                  <div className="guest-houses">Guest houses</div>
                </div>
                <div className="ul14">
                  <div className="unique-places-to">Unique places to stay</div>
                  <div className="reviews2">Reviews</div>
                  <div className="discover-monthly-stays">
                    Discover monthly stays
                  </div>
                  <div className="unpacked-travel-articles">
                    Unpacked: Travel articles
                  </div>
                  <div className="travel-communities">Travel Communities</div>
                  <div className="seasonal-and-holiday">
                    Seasonal and holiday deals
                  </div>
                  <div className="traveller-review-awards">
                    Traveller Review Awards
                  </div>
                </div>
                <div className="ul15">
                  <div className="car-hire">Car hire</div>
                  <div className="flight-finder">Flight finder</div>
                  <div className="restaurant-reservations">
                    Restaurant reservations
                  </div>
                  <div className="bookingcom-for-travel">
                    Booking.com for Travel Agents
                  </div>
                </div>
                <div className="ul16">
                  <div className="coronavirus-covid-19-faqs">
                    Coronavirus (COVID-19) FAQs
                  </div>
                  <div className="about-bookingcom">About Booking.com</div>
                  <div className="customer-service-help1">
                    Customer Service help
                  </div>
                  <div className="partner-help">Partner help</div>
                  <div className="careers1">Careers</div>
                  <div className="sustainability">Sustainability</div>
                  <div className="press-centre">Press centre</div>
                  <div className="safety-resource-centre">
                    Safety resource centre
                  </div>
                  <div className="investor-relations1">Investor relations</div>
                  <div className="terms-conditions1">{`Terms & Conditions`}</div>
                  <div className="partner-dispute">Partner dispute</div>
                  <div className="how-we-work">How we work</div>
                  <div className="privacy-cookie">{`Privacy & Cookie Statement`}</div>
                  <div className="do-not-sell">
                    Do not sell or share my personal information
                  </div>
                  <div className="msa-statement">MSA Statement</div>
                  <div className="corporate-contact">Corporate contact</div>
                </div>
              </div>
              <div className="div879">
                <div className="div880">
                  <div className="extranet-login">Extranet login</div>
                </div>
                <div className="copyright-19962023">
                  Copyright © 1996–2023 Booking.com™. All rights reserved.
                </div>
              </div>
              <div className="div881">
                <div className="bookingcom-is-part">
                  Booking.com is part of Booking Holdings Inc., the world leader
                  in online travel and related services.
                </div>
                <div className="ul17">
                  <img
                    className="c8d1832de6a3123b6ee45b59ae2f81-icon"
                    alt=""
                    src="../27c8d1832de6a3123b6ee45b59ae2f81b0d9d0d0png@2x.png"
                  />
                  <img
                    className="f80e129541f2a952d470df24473733-icon"
                    alt=""
                    src="../27c8d1832de6a3123b6ee45b59ae2f81b0d9d0d0png@2x.png"
                  />
                  <img
                    className="ef7122074473a6566094e957ff834b-icon"
                    alt=""
                    src="../83ef7122074473a6566094e957ff834badb58ce6png@2x.png"
                  />
                  <img
                    className="c9191b6a3651bf030e41e99a153b64-icon"
                    alt=""
                    src="../1c9191b6a3651bf030e41e99a153b64f449845edpng@2x.png"
                  />
                  <img
                    className="bc5ec89d870111592a378bbe7a2086-icon"
                    alt=""
                    src="../6bc5ec89d870111592a378bbe7a2086f0b01abc4png@2x.png"
                  />
                  <img
                    className="a4b50503eda6c15773d6e61c238230-icon"
                    alt=""
                    src="../a4b50503eda6c15773d6e61c238230eb42fb050dpng@2x.png"
                  />
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="text" />
        <img
          className="wwwbookingcom-by-htmltodes-child"
          alt=""
          src="../group-1323.svg"
        />
        <div className="locate4">Locate</div>
        <div className="about3">About</div>
        <div className="contact-us4">Contact us</div>
        <div className="wwwbookingcom-by-htmltodes-inner">
          <div className="group-wrapper417">
            <div className="group-wrapper417">
              <div className="group-wrapper417">
                <div className="group-wrapper417">
                  <div className="group-wrapper417">
                    <div className="group-wrapper417">
                      <div className="group-wrapper417">
                        <div className="group-wrapper417">
                          <div className="group-wrapper417">
                            <div className="group-wrapper417">
                              <div className="group-wrapper417">
                                <div className="group-wrapper417">
                                  <div className="group-wrapper417">
                                    <div className="group-wrapper417">
                                      <div className="group-wrapper417">
                                        <div className="group-wrapper417">
                                          <div className="group-wrapper417">
                                            <div className="group-wrapper417">
                                              <div className="group-wrapper417">
                                                <div className="group-wrapper417">
                                                  <div className="group-wrapper417">
                                                    <div className="group-wrapper417">
                                                      <div className="group-wrapper417">
                                                        <div className="group-wrapper417">
                                                          <div className="group-wrapper417">
                                                            <div className="group-wrapper417">
                                                              <div className="group-wrapper417">
                                                                <div className="group-wrapper417">
                                                                  <div className="group-wrapper417">
                                                                    <div className="group-wrapper417">
                                                                      <div className="group-wrapper417">
                                                                        <div className="group-wrapper417">
                                                                          <div className="group-wrapper417">
                                                                            <div className="group-wrapper417">
                                                                              <div className="group-wrapper417">
                                                                                <div className="group-wrapper417">
                                                                                  <div className="group-wrapper417">
                                                                                    <div className="group-wrapper417">
                                                                                      <div className="group-wrapper417">
                                                                                        <div className="group-wrapper417">
                                                                                          <div className="group-wrapper459">
                                                                                            <div className="group-wrapper460">
                                                                                              <div className="group-wrapper460">
                                                                                                <div className="group-wrapper460">
                                                                                                  <div className="group-wrapper460">
                                                                                                    <div className="group-wrapper460">
                                                                                                      <div className="group-wrapper460">
                                                                                                        <div className="group-wrapper460">
                                                                                                          <div className="group-wrapper460">
                                                                                                            <div className="group-wrapper460">
                                                                                                              <div className="group-wrapper460">
                                                                                                                <div className="group-wrapper460">
                                                                                                                  <div className="group-wrapper460">
                                                                                                                    <div className="group-wrapper460">
                                                                                                                      <div className="group-wrapper460">
                                                                                                                        <div className="group-wrapper460">
                                                                                                                          <div className="group-wrapper460">
                                                                                                                            <div className="group-wrapper460">
                                                                                                                              <div className="group-wrapper460">
                                                                                                                                <div className="group-wrapper460">
                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                                  <div className="group-wrapper460">
                                                                                                                                                                                                    <div className="group-wrapper460">
                                                                                                                                                                                                      <div className="group-wrapper460">
                                                                                                                                                                                                        <div className="group-wrapper460">
                                                                                                                                                                                                          <div className="group-wrapper460">
                                                                                                                                                                                                            <div className="group-wrapper460">
                                                                                                                                                                                                              <div className="group-wrapper460">
                                                                                                                                                                                                                <div className="group-wrapper460">
                                                                                                                                                                                                                  <div className="hostel-hopper4">
                                                                                                                                                                                                                    Hostel
                                                                                                                                                                                                                    Hopper
                                                                                                                                                                                                                  </div>
                                                                                                                                                                                                                </div>
                                                                                                                                                                                                              </div>
                                                                                                                                                                                                            </div>
                                                                                                                                                                                                          </div>
                                                                                                                                                                                                        </div>
                                                                                                                                                                                                      </div>
                                                                                                                                                                                                    </div>
                                                                                                                                                                                                  </div>
                                                                                                                                                                                                </div>
                                                                                                                                                                                              </div>
                                                                                                                                                                                            </div>
                                                                                                                                                                                          </div>
                                                                                                                                                                                        </div>
                                                                                                                                                                                      </div>
                                                                                                                                                                                    </div>
                                                                                                                                                                                  </div>
                                                                                                                                                                                </div>
                                                                                                                                                                              </div>
                                                                                                                                                                            </div>
                                                                                                                                                                          </div>
                                                                                                                                                                        </div>
                                                                                                                                                                      </div>
                                                                                                                                                                    </div>
                                                                                                                                                                  </div>
                                                                                                                                                                </div>
                                                                                                                                                              </div>
                                                                                                                                                            </div>
                                                                                                                                                          </div>
                                                                                                                                                        </div>
                                                                                                                                                      </div>
                                                                                                                                                    </div>
                                                                                                                                                  </div>
                                                                                                                                                </div>
                                                                                                                                              </div>
                                                                                                                                            </div>
                                                                                                                                          </div>
                                                                                                                                        </div>
                                                                                                                                      </div>
                                                                                                                                    </div>
                                                                                                                                  </div>
                                                                                                                                </div>
                                                                                                                              </div>
                                                                                                                            </div>
                                                                                                                          </div>
                                                                                                                        </div>
                                                                                                                      </div>
                                                                                                                    </div>
                                                                                                                  </div>
                                                                                                                </div>
                                                                                                              </div>
                                                                                                            </div>
                                                                                                          </div>
                                                                                                        </div>
                                                                                                      </div>
                                                                                                    </div>
                                                                                                  </div>
                                                                                                </div>
                                                                                              </div>
                                                                                            </div>
                                                                                          </div>
                                                                                          <img
                                                                                            className="bunk-1-icon3"
                                                                                            alt=""
                                                                                            src="../bunk-14@2x.png"
                                                                                          />
                                                                                          <div className="group-wrapper518">
                                                                                            <div className="group-wrapper518">
                                                                                              <div className="group-wrapper518">
                                                                                                <div className="group-wrapper518">
                                                                                                  <div className="group-wrapper518">
                                                                                                    <div className="group-wrapper518">
                                                                                                      <div className="group-wrapper518">
                                                                                                        <div className="group-wrapper518">
                                                                                                          <div className="group-wrapper518">
                                                                                                            <div className="group-wrapper518">
                                                                                                              <div className="group-wrapper518">
                                                                                                                <div className="group-wrapper518">
                                                                                                                  <div className="group-wrapper518">
                                                                                                                    <div className="group-wrapper518">
                                                                                                                      <div className="group-wrapper518">
                                                                                                                        <div className="group-wrapper518">
                                                                                                                          <div className="group-wrapper518">
                                                                                                                            <div className="group-wrapper518">
                                                                                                                              <div className="group-wrapper518">
                                                                                                                                <div className="group-wrapper518">
                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                            <div className="group-wrapper518">
                                                                                                                                              <div className="group-wrapper518">
                                                                                                                                                <div className="group-wrapper518">
                                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                                            <div className="group-wrapper518">
                                                                                                                                                              <div className="group-wrapper518">
                                                                                                                                                                <div className="group-wrapper518">
                                                                                                                                                                  <div className="group-wrapper518">
                                                                                                                                                                    <div className="group-wrapper518">
                                                                                                                                                                      <div className="group-wrapper518">
                                                                                                                                                                        <div className="group-wrapper518">
                                                                                                                                                                          <div className="group-wrapper518">
                                                                                                                                                                            <img
                                                                                                                                                                              className="pngwing-1-icon3"
                                                                                                                                                                              alt=""
                                                                                                                                                                              src="../pngwing-14@2x.png"
                                                                                                                                                                            />
                                                                                                                                                                          </div>
                                                                                                                                                                        </div>
                                                                                                                                                                      </div>
                                                                                                                                                                    </div>
                                                                                                                                                                  </div>
                                                                                                                                                                </div>
                                                                                                                                                              </div>
                                                                                                                                                            </div>
                                                                                                                                                          </div>
                                                                                                                                                        </div>
                                                                                                                                                      </div>
                                                                                                                                                    </div>
                                                                                                                                                  </div>
                                                                                                                                                </div>
                                                                                                                                              </div>
                                                                                                                                            </div>
                                                                                                                                          </div>
                                                                                                                                        </div>
                                                                                                                                      </div>
                                                                                                                                    </div>
                                                                                                                                  </div>
                                                                                                                                </div>
                                                                                                                              </div>
                                                                                                                            </div>
                                                                                                                          </div>
                                                                                                                        </div>
                                                                                                                      </div>
                                                                                                                    </div>
                                                                                                                  </div>
                                                                                                                </div>
                                                                                                              </div>
                                                                                                            </div>
                                                                                                          </div>
                                                                                                        </div>
                                                                                                      </div>
                                                                                                    </div>
                                                                                                  </div>
                                                                                                </div>
                                                                                              </div>
                                                                                            </div>
                                                                                          </div>
                                                                                        </div>
                                                                                      </div>
                                                                                    </div>
                                                                                  </div>
                                                                                </div>
                                                                              </div>
                                                                            </div>
                                                                          </div>
                                                                        </div>
                                                                      </div>
                                                                    </div>
                                                                  </div>
                                                                </div>
                                                              </div>
                                                            </div>
                                                          </div>
                                                        </div>
                                                      </div>
                                                    </div>
                                                  </div>
                                                </div>
                                              </div>
                                            </div>
                                          </div>
                                        </div>
                                      </div>
                                    </div>
                                  </div>
                                </div>
                              </div>
                            </div>
                          </div>
                        </div>
                      </div>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
        <div className="group-parent8">
          <div className="rectangle-container">
            <div className="group-child1" />
            <div className="search-for-files1">
              Search for files, plugins, and creators
            </div>
          </div>
          <img className="group-child4" alt="" src="../group-153.svg" />
          <div className="search7">search</div>
        </div>
      </div>
    </div>
  );
};

export default WwwstanzalivingcomByHtmlt;
